...
class TaskController extends FrontController
{
          ...
          */
    	    ...
    	    {    
                ...
                # Филтрации [sort]
                $sort = !empty($args[0]) ? (string) $args[0] : -1;
               
                # извлекаем все данные и передаю в виду
                $tasks = $this->task->all("$start, $perpage", $sort)->results();
          ...
          */
          private function form($task = null)
          {
              ...
              $this->render('partials/task/form', compact('form', 'errors', 'task', 'url', 'status'));
          }
